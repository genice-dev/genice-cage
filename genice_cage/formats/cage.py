...
def hook6(lattice):
    ...
    for cage in options.cages:
        ...
        for j, mol in enumerate(cagemols):
            for k in range(4):
                resno, resname, atomname, position, order = lattice.atoms[mol*4+k]
                if resno == 0:
                    molorder += 1
                if len(lattice.atoms) > 99999:
                    f += "{0:5d}{1:5s}{2:>5s}{3:5d}{4:8.3f}{5:8.3f}{6:8.3f}\n".format(9999,resname, atomname, 9999,position[0],position[1],position[2])
                else:
                    f += "{0:5d}{1:5s}{2:>5s}{3:5d}{4:8.3f}{5:8.3f}{6:8.3f}\n".format(molorder,resname, atomname, atomcount+1,position[0],position[1],position[2])
                atomcount += 1
        s += "{0}\n".format(atomcount) + f
